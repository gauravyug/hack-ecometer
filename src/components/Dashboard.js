    ...
    <div className="container mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-md text-center">
          ...
          />
          <h3 className="mt-4 text-lg font-semibold">Total Footprint</h3>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md text-center">
          <CircularProgressbar
            ...
            styles={buildStyles({
              textColor: '#333',
              pathColor: `rgba(76, 175, 80, ${goalProgressPercentage / 100})`,
              trailColor: '#e0e0e0'
            })}
          />
          <h3 className="mt-4 text-lg font-semibold">Goal Progress</h3>
          {goal && <p className="text-gray-500">Goal: {goal.toFixed(2)} kg CO₂</p>}
        </div>
      </div>

      <div className="flex justify-center space-x-4">
        {['day', 'week', 'lifetime'].map(frame => (
          <button
            key={frame}
            onClick={() => setTimeframe(frame)}
            className={`px-4 py-2 rounded-lg font-semibold ${timeframe === frame ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
          >
      ...
      {filteredActivities.length > 0 && (
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">Footprint by Category</h2>
      ...
      <div className="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md space-y-6">
        <h2 className="text-xl font-semibold text-center">Log Your Activities</h2>

        <div>
          <label className="block text-gray-700 mb-2">Scan with Image</label>
          <input
            type="file"
            accept="image/*"
        ...
        <div>
          <label className="block text-gray-700 mb-2">{getPlaceholderText()}</label>
          <input
            type="number"
            value={amount}
        ...
        <div className="text-center">
          <p className="text-gray-700 font-bold">Estimated Footprint: <span className="text-green-600">{footprint} kg CO₂</span></p>
        </div>

        <button
          onClick={logActivity}
          className="w-full p-3 bg-green-500 text-white font-bold rounded-md hover:bg-green-600 transition-colors"
        >
          Log Activity
        </button>
      </div>
